...
module.exports.create = function ( initialBalance ) {
	...
	AccountManager.prototype.getStatement = function ( account ) {
		...
		statement.balance = Math.abs(balance).toFixed(2);
		statement.transactions = [];
		statement.title = accounts[account].name;
		statement.negativeBalance = (balance < 0);
		...
		for ( var id = 0; id < limit; id++ ) {
			...
			if ( transaction.sourceAccount == account || transaction.destinationAccount == account ) {
				...
				statementEntry.amount = transaction.amount.toFixed(2);
				...
				statementEntry.balance = Math.abs(balance).toFixed(2);
				statementEntry.negativeBalance = (balance < 0);

				statement.transactions.push ( statementEntry );
			}
		}
		return statement;
	};
